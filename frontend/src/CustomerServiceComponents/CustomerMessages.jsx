...
const CustomerMessages = () => {
  ...
  // Fetch all past messages on mount (support POV)
  useEffect(() => {
      ...
      try {
        ...
        if (Array.isArray(data)) {
          const formatted = data.map((msg) => ({
            text: msg.message,
            // For support POV: support's messages on right, customer's on left
            side: msg.sender === "support" ? "right" : "left",
          }));
  ...
  const sendMessage = async () => {
    ...
    const payload = {
      ...
      supportId,
      message: inputValue,
      sender: "support", // Explicitly set sender as support
    };
    ...
    try {
      ...
      // Append locally as support's own message (right)
